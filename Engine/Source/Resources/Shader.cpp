...
namespace NL
{
	std::unordered_map<std::string, ShaderUsage> Shader::s_ShaderUsageMap =
	{
		{"model", ShaderUsage::Model},
		{"post", ShaderUsage::PostProcessing},
		{"partical", ShaderUsage::ParticalSystem},
	};
	...
	bool Shader::ShaderFileParser(const std::string& rawSrc, std::string& vertexSrc, std::string& fragmentSrc, std::string& geometrySrc)
	{
		...
		// Parsing usage
		{
			const char* usageToken = "#use";
			size_t usageTokenLength = strlen(usageToken);
			size_t usagePos = rawSrc.find(usageToken, 0);
			if (usagePos != std::string::npos)
			{
				size_t eol = rawSrc.find_first_of("\r\n", usagePos);
				size_t begin = rawSrc.find_first_not_of(" ", usagePos + usageTokenLength);
				std::string usage = rawSrc.substr(begin, eol - begin);
				if (s_ShaderUsageMap.contains(usage))
				{
					m_Usage = s_ShaderUsageMap[usage];
					NL_ENGINE_INFO("Shader name: {0}, use: {1}", rawSrc, usage);
				}
			}
		}
	...
	ShaderUsage Shader::ParseShaderUsage(const std::string& path)
	{
		std::string rawSrc = ReadShaderFile(path);
		ShaderUsage shaderUsage = ShaderUsage::Unknown;
		...
		if (usagePos != std::string::npos)
		{
			...
			if (s_ShaderUsageMap.contains(usage))
			{
				shaderUsage = s_ShaderUsageMap[usage];
				NL_ENGINE_INFO("Shader name: {0}, use: {1}", rawSrc, usage);
			}
		}
