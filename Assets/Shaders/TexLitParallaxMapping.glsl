...
vec2 ParallaxMapping(vec3 viewDirTS)
{
	float depth = texture(u_HeightMap, v_TexCoord).r;
	return v_TexCoord - viewDirTS.xy * depth / viewDirTS.z * u_ParallaxParam;
}
...
void main()
{
	...
	vec2 texCoord = ParallaxMapping(viewDirTS);
	//texCoord = v_TexCoord;
